...
import { extractSuiValues } from './caip.js';
/**
 ...
 */
export async function computeInvoiceHash(invoiceJwt) {
 ...
 * - Not expired
 */
export async function verifyPaymentPTB(ptbBytes, invoice, invoiceJwt) {
    try {
        ...
        for (const cmd of commands) {
            ...
            if (!Object.values(AllowedCommand).includes(cmdType)) {
                return {
                    pass: false,
        ...
        const moveCalls = commands.filter((cmd) => cmd.$kind === 'MoveCall');
        ...
        const transfers = commands.filter((cmd) => cmd.$kind === 'TransferObjects');
        ...
        if (hasSettlePaymentCall) {
            ...
            const settleCall = moveCalls.find((call) => {
                const moveCall = call.MoveCall;
        ...
        const transferRecipients = transfers.map((t) => {
        ...
        if (!hasSettlePaymentCall) {
            ...
            const hasMerchantTransfer = transferRecipients.some(addr => addr === effectiveMerchant);
        ...
        const unauthorizedRecipients = transferRecipients.filter(addr => addr && !authorizedRecipients.includes(addr));
        ...
        const splits = commands.filter((cmd) => cmd.$kind === 'SplitCoins');
 ...
 */
export function verifyPaymentPTBBasic(ptbBytes, expectedRecipient) {
    try {
        const tx = Transaction.from(ptbBytes);
        const data = tx.getData();
        const commands = data.commands || [];
